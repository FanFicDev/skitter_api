#!/usr/bin/env python3
from typing import TYPE_CHECKING, Optional, List, Tuple, Dict, Any, cast
import base64
import json
import sys
import subprocess
...
logFileName = 'skitter_story_worker.log'
...
def restart_worker_fr(node: str) -> None:
	plog(f'restart_worker_fr: restarting {node}_flaresolverr2')
	run_command(['docker', 'restart', f'{node}_flaresolverr2'], timeout=60)
	time.sleep(30)

def get_fr_ok(workerId: int) -> bool:
	global defaultRequestTimeout
	host = get_node_host(workerId)
	r = requests.get(f'http://{host}', timeout=defaultRequestTimeout)
	if r.status_code != 200:
		return False
	try:
		ro = r.json()
		return str(ro['msg']) == 'FlareSolverr is ready!'
	except:
		pass
	return False
...
def make_req(workerId: int, url: str, headers: Optional[Dict[str, str]] = None,
	...
	global defaultRequestTimeout
	host = get_node_host(workerId)
	data: Dict[str, Any] = {'cmd':'request.get','session':'skitter','url':url}
...
def get_worker_ip_fr(workerId: int) -> str:
	...
	res = r.response.decode('utf-8').strip()
	PREFIX = '<body><pre>'
	if res.find(PREFIX) < 0:
		raise Exception('get_worker_ip_fr: malformed response')
	res = res[res.find(PREFIX) + len(PREFIX):]
	SUFFIX = '</pre></body>'
	if res.find(SUFFIX) < 0 or res.find(SUFFIX) > 16:
		raise Exception('get_worker_ip_fr: malformed response')
	res = res[:res.find(SUFFIX)]
	return res
...
def workBlock(workerId: int, stripeCount: int, stripe: int, cnt: int,
	...
	while cnt > 0 and timeout > 0:
		wq = WebQueue.next(oil.open(), workerId, 2, stripeCount=stripeCount, stripe=stripe)
		if wq is None:
			timeout -= idleTimeout
...
def work(workerId: int, stripeCount: int, stripe: int, blockSize: int) -> int:
	...
	while True:
		...
		source = WebSource.lookup(oil.open(), f'{NODE_NAME}_{node}_fr2', remoteIP)
